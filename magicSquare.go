...
integers from 1 to n^2 where the sum of any row, column, or diagonal
of length n is always equal to the same number: the magic constant.
You will be given a 3 x 3 matrix s of integers in the inclusive range
[1, 9]. We can convert any digit a to any other digit b in the range
[1, 9] at cost of |a - b|. Given s, convert it into a magic square at
minimal cost. Print this cost on a new line.
Note: The resulting magic square must contain distinct integers in
the inclusive range [1, 9].
...
Returns
int: the minimal total cost of converting the input square to a magic
square
...
func formingMagicSquare(s [][]int32) int32 {
	...
	var allMagicSquares [][][]int32 = [][][]int32{
		{{8, 1, 6}, {3, 5, 7}, {4, 9, 2}},
		{{6, 1, 8}, {7, 5, 3}, {2, 9, 4}},
		{{2, 7, 6}, {9, 5, 1}, {4, 3, 8}},
		{{4, 3, 8}, {9, 5, 1}, {2, 7, 6}},
	...
	var minCost int32 = 72
	for i := range allMagicSquares {
		var cost int32
		for j := range allMagicSquares[i] {
			for k := range allMagicSquares[i][j] {
				if allMagicSquares[i][j][k] > s[j][k] {
					cost += (allMagicSquares[i][j][k] - s[j][k])
				} else {
					cost += (s[j][k] - allMagicSquares[i][j][k])
				}
			}
		}
